...
void to_json(json& j, const std::shared_ptr<ConnectorTransactionHandle>& p) {
  ...
  if (getConnectorKey(type) == "iceberg") {
    j = *std::static_pointer_cast<IcebergTransactionHandle>(p);
    return;
  }
  if (getConnectorKey(type) == "tpch") {
...
void from_json(const json& j, std::shared_ptr<ConnectorTransactionHandle>& p) {
  ...
  if (getConnectorKey(type) == "iceberg") {
    auto k = std::make_shared<IcebergTransactionHandle>();
    j.get_to(*k);
    p = k;
    return;
  }
...
namespace facebook::presto::protocol {
IcebergTransactionHandle::IcebergTransactionHandle() noexcept {
  _type = "iceberg";
}

void to_json(json& j, const IcebergTransactionHandle& p) {
  j = json::object();
  j["@type"] = "iceberg";
  to_json_key(j, "uuid", p.uuid, "IcebergTransactionHandle", "UUID", "uuid");
}

void from_json(const json& j, IcebergTransactionHandle& p) {
  p._type = j["@type"];
  from_json_key(j, "uuid", p.uuid, "IcebergTransactionHandle", "UUID", "uuid");
}
} // namespace facebook::presto::protocol
namespace facebook::presto::protocol {
